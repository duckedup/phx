...
#[test]
fn auto_commit_dirty_worktree() {
    let dir = init_test_repo();
    let mgr = WorktreeManager::new(dir.path().to_path_buf()).unwrap();

    let info = mgr.create("c-02").unwrap();

    let committed = mgr.auto_commit("c-02", "no-op").unwrap();
    assert!(!committed);

    std::fs::write(info.path.join("new_file.txt"), "hello").unwrap();
    let committed = mgr.auto_commit("c-02", "added file").unwrap();
    assert!(committed);

    mgr.remove("c-02", true).unwrap();
}

#[test]
fn diff_summary_shows_changes() {
    let dir = init_test_repo();
    let mgr = WorktreeManager::new(dir.path().to_path_buf()).unwrap();

    let info = mgr.create("c-03").unwrap();
    std::fs::write(info.path.join("feature.rs"), "fn main() {}").unwrap();
    mgr.auto_commit("c-03", "add feature").unwrap();

    let diff = mgr.diff_summary("c-03", "main").unwrap();
    assert!(
    ...
    mgr.remove("c-03", true).unwrap();
}

#[test]
fn merge_squash_and_cleanup() {
    let dir = init_test_repo();
    let mgr = WorktreeManager::new(dir.path().to_path_buf()).unwrap();

    let info = mgr.create("c-04").unwrap();
    std::fs::write(info.path.join("merged.txt"), "content").unwrap();
    mgr.auto_commit("c-04", "work").unwrap();
...
fn cleanup_all_removes_everything() {
    ...
    mgr.create("c-10").unwrap();
    mgr.create("c-11").unwrap();
    mgr.create("c-12").unwrap();
